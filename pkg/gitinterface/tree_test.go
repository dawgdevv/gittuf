...
func TestNewEntryBlobWithPermissions(t *testing.T) {
	...
	entry := NewEntryBlobWithPermissions("a", blobID, 0o755)
	assert.Equal(t, "a", entry.getName())
...
func TestNewEntryTree(t *testing.T) {
	...
	treeID, err := treeBuilder.WriteTreeFromEntries([]TreeEntry{NewEntryBlob("a", blobID)})
	if err != nil {
		t.Fatal(err)
	}

	entryWithNil := NewEntryTree("foo", nil)
	assert.Equal(t, "foo", entryWithNil.getName())

	entryWithZero := NewEntryTree("foo", ZeroHash)
	assert.Equal(t, "foo", entryWithZero.getName())

	entryWithValid := NewEntryTree("foo", treeID)
	assert.Equal(t, "foo", entryWithValid.getName())
}
